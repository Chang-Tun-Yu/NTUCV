		...
		class Decoder(torch.nn.Module):
			...
			def forward(self, tensorFirst, tensorSecond, objectPrevious,  Backward_tensorGrid, Backward_tensorPartial):
				...
				if objectPrevious is None:
					tensorFlow = None
					tensorFeat = None
					tmp = correlation.FunctionCorrelation(tensorFirst=tensorFirst.cuda(), tensorSecond=tensorSecond.cuda())
					tensorVolume = torch.nn.functional.leaky_relu(input=tmp.cpu(), negative_slope=0.1, inplace=False)
					...
					cuda_tensorFlow = tensorFlow.cuda()
					back_tmp = self.Backward(tensorSecond.cuda(), cuda_tensorFlow * self.dblBackward, Backward_tensorGrid, Backward_tensorPartial)
					tmp = correlation.FunctionCorrelation(tensorFirst=tensorFirst.cuda(), tensorSecond=back_tmp)
					tensorVolume = torch.nn.functional.leaky_relu(input=tmp.cpu(), negative_slope=0.1, inplace=False)
